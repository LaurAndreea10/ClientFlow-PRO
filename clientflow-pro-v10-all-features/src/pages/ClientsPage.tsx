...
import { bulkDeleteClients, bulkUpdateClientsStatus, createClient, deleteClient, getClients, importClientsCsv, updateClient } from '../lib/mockApi'
...
export function ClientsPage() {
  ...
  const createMutation = useMutation({ mutationFn: createClient, onSuccess: () => { refresh(); setForm(defaultForm); pushToast('Client added', 'success') } })
  const updateMutation = useMutation({ mutationFn: ({ id, payload }: { id: string; payload: Partial<Client> }) => updateClient(id, payload), onSuccess: () => { refresh(); setEditingId(null); setForm(defaultForm); pushToast('Client updated', 'success') } })
  const deleteMutation = useMutation({ mutationFn: deleteClient, onSuccess: () => { refresh(); pushToast('Client deleted', 'warning') } })
    ...
    <div className="grid page-gap-lg">
      <div className="page-header stack-on-mobile">
        <div>
          ...
          <p className="muted">Search, sort, bulk edit, and import records from CSV in a mobile-first workflow.</p>
        </div>
        ...
        <div className="card card-pad">
          ...
          {selectedIds.length ? (
            <div className="toolbar wrap-row bulk-bar">
              ...
              <button className="button danger" type="button" onClick={() => bulkDeleteMutation.mutate(selectedIds)}>Delete selected</button>
            </div>
          ) : null}
              ...
              <article key={client.id} className="mobile-list-card">
                ...
                <div className="mobile-list-row" style={{ marginTop: 12 }}>
                  ...
                  <div className="toolbar compact-toolbar">
                    ...
                    <button className="button danger" onClick={() => deleteMutation.mutate(client.id)} type="button">Delete</button>
                  </div>
                </div>
              </article>
            ))}
            {!isLoading && filtered.length === 0 ? <div className="empty-state premium-empty-state"><div className="empty-icon">🔎</div><strong>No clients match your filters.</strong><div className="small muted">Try another status, clear search, or import a CSV.</div></div> : null}
          </div>
        </div>
      </div>
    </div>

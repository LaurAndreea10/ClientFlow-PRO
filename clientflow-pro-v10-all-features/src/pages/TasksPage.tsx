...
import { bulkDeleteTasks, bulkUpdateTasksStatus, createTask, deleteTask, getClients, getTasks, importTasksCsv, updateTask } from '../lib/mockApi'
import { LoadingBlock } from '../components/ui/LoadingBlock'
import { useToast } from '../features/toast/ToastContext'
import type { Task, TaskPriority, TaskStatus } from '../types'

const defaultForm = {
  title: '', description: '', clientId: '', priority: 'medium' as TaskPriority, status: 'todo' as TaskStatus, dueDate: '',
}
const kanbanColumns: Array<{ status: TaskStatus; label: string; icon: string }> = [
...
export function TasksPage() {
  ...
  const refresh = () => {
    for (const key of ['tasks', 'dashboard', 'reports', 'calendar', 'activity', 'notifications']) {
      queryClient.invalidateQueries({ queryKey: [key] })
    }
  }

  const createMutation = useMutation({ mutationFn: createTask, onSuccess: () => { refresh(); setForm(defaultForm); pushToast('Task created', 'success') } })
  const updateMutation = useMutation({ mutationFn: ({ id, payload }: { id: string; payload: Partial<Task> }) => updateTask(id, payload), onSuccess: () => { refresh(); setEditingId(null); setForm(defaultForm); pushToast('Task updated', 'success') } })
  const deleteMutation = useMutation({ mutationFn: deleteTask, onSuccess: () => { refresh(); pushToast('Task deleted', 'warning') } })
  ...
  const bulkStatusMutation = useMutation({ mutationFn: ({ ids, status }: { ids: string[]; status: TaskStatus }) => bulkUpdateTasksStatus(ids, status), onSuccess: () => { refresh(); setSelectedIds([]); pushToast('Bulk task update done', 'success') } })
  ...
  const tasksWithClient = useMemo(() => {
    const mapped = tasks.map((task) => ({ ...task, clientName: clients.find((client) => client.id === task.clientId)?.name ?? 'General' }))
      .filter((task) => (statusFilter === 'all' || task.status === statusFilter) && (priorityFilter === 'all' || task.priority === priorityFilter))
    if (sortBy === 'priority') return mapped.sort((a, b) => ['high', 'medium', 'low'].indexOf(a.priority) - ['high', 'medium', 'low'].indexOf(b.priority))
    if (sortBy === 'newest') return mapped.sort((a, b) => +new Date(b.createdAt) - +new Date(a.createdAt))
    return mapped.sort((a, b) => (a.dueDate || '9999').localeCompare(b.dueDate || '9999'))
  }, [tasks, clients, statusFilter, priorityFilter, sortBy])

  const kanbanBuckets = kanbanColumns.map((column) => ({ ...column, items: tasksWithClient.filter((task) => task.status === column.status) }))
  ...
  function startEdit(task: Task) { setEditingId(task.id); setForm({ title: task.title, description: task.description, clientId: task.clientId ?? '', priority: task.priority, status: task.status, dueDate: task.dueDate }); window.scrollTo({ top: 0, behavior: 'smooth' }) }
  ...
  async function handleImportFile(file: File) { importMutation.mutate(await file.text()) }
              ...
              {tasksWithClient.map((task) => (
                <article key={task.id} className="mobile-list-card">
                  ...
                  <div className="mobile-list-row wrap-row">
                    ...
                    <div className="toolbar compact-toolbar"><button className="button secondary" onClick={() => startEdit(task)} type="button">Edit</button><button className="button danger" onClick={() => deleteMutation.mutate(task.id)} type="button">Delete</button></div>
                  </div>
                </article>
              ))}

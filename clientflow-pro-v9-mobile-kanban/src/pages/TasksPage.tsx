...
export function TasksPage() {
  ...
  const tasksWithClient = useMemo(() => {
    ...
    return tasks
      .map((task) => ({
        ...task,
        clientName: task.clientId ? map.get(task.clientId) ?? 'Unknown client' : 'General',
      }))
  ...
  const kanbanBuckets = useMemo(() => {
    return kanbanColumns.map((column) => ({
      ...column,
      items: tasksWithClient.filter((task) => task.status === column.status),
    }))
  }, [tasksWithClient])
  ...
  function handleDrop(targetStatus: TaskStatus) {
    ...
    updateMutation.mutate({ id: draggingTaskId, payload: { status: targetStatus } })
  }
    ...
    <div className="grid page-gap-lg">
      ...
      <div className="two-col two-col-equal mobile-reverse">
        ...
        <div className="card card-pad">
          <div className="card-title-row stack-on-mobile">
            <div>
              <h2 style={{ margin: 0 }}>{viewMode === 'kanban' ? 'Kanban workflow' : 'Task list'}</h2>
              <div className="small muted">Filter by status and priority, then edit inline or drag cards across columns.</div>
            </div>
              ...
              {kanbanBuckets.map((column) => (
                ...
                >
                  <div className="kanban-column-header">
                    <div>
                      <div className="small muted">{column.icon} {column.label}</div>
                      <h3 style={{ margin: '4px 0 0' }}>{column.items.length} task{column.items.length === 1 ? '' : 's'}</h3>
                    </div>
                    <span className="pill small-pill">{column.status.replace('_', ' ')}</span>
                  </div>
                  ...
                  <div className="kanban-stack">
                    {column.items.length ? column.items.map((task) => (
                      <article
                        key={task.id}
                        className={`kanban-card ${draggingTaskId === task.id ? 'dragging' : ''}`}
                        draggable
                      ...
                      >
                        ...
                        <div className="mobile-list-row wrap-row">
                          ...
                          <div className="toolbar compact-toolbar wrap-row">
                            <button className="button secondary" onClick={() => startEdit(task)} type="button">Edit</button>
                            <button className="button danger" onClick={() => deleteMutation.mutate(task.id)} type="button">Delete</button>
                          </div>
                        </div>
                      </article>
                    )) : (
                      <div className="empty-dropzone">
                        ...
                        <div className="small muted">Move cards to change workflow status instantly.</div>
                      </div>
                    )}
                  </div>
                </section>

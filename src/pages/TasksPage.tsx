...
export function TasksPage() {
  ...
  const archiveMutation = useMutation({ mutationFn: archiveTask, onSuccess: refreshTasks })
  const restoreMutation = useMutation({ mutationFn: restoreTask, onSuccess: refreshTasks })
  ...
  const filteredTasks = useMemo(() => {
    ...
    return tasks.filter((task) => {
      ...
      return matchesArchive && matchesPriority && matchesSearch
    })
            ...
            return (
              <div className="kanban-column" key={column.id}>
                <div className="card-title-row">
                  <div>
                    <h2 style={{ margin: 0 }}>{column.label}</h2>
                    <div className="small muted">{column.hint}</div>
                  </div>
                ...
                <div className="list">
                  {columnTasks.map((task) => (
                    <TaskCard
                      key={task.id}
                      task={task}
                      clientName={task.clientId ? clientsById.get(task.clientId) ?? 'Unknown client' : 'General'}
                      onMove={(id, nextStatus) => updateMutation.mutate({ id, nextStatus })}
                      onArchive={(id) => archiveMutation.mutate(id)}
                      onRestore={(id) => restoreMutation.mutate(id)}
                      onAddSubtask={(id, nextTitle) => addSubtaskMutation.mutate({ id, title: nextTitle })}
                      onToggleSubtask={(taskId, subtaskId) => toggleSubtaskMutation.mutate({ taskId, subtaskId })}
                      onAddComment={(id, content) => addCommentMutation.mutate({ id, content })}
                    />
                  ))}
        ...
        <aside className="card card-pad sticky-action-panel">
          <div className="card-title-row">
            <div>
              ...
              <div className="small muted">Create tagged, recurring work in one flow.</div>
            </div>
          </div>
          <form className="form-grid" onSubmit={handleSubmit}>
            ...
            <select className="select" value={recurrence} onChange={(e) => setRecurrence(e.target.value as RecurrenceRule)}>
